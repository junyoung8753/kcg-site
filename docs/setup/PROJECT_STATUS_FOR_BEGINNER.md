...
- 배포된 것: `v0.2.30`은 기존 개인 Vercel Hobby project `kcg-confirm-preview`로 production review deploy 됐다. 현재 deployment id는 `dpl_8hTZ3f1pLu3nnoN4W48F2KMA8Lyb`이고, `https://kcgold.co.kr`, `https://www.kcgold.co.kr`, `https://kcg-confirm-preview.vercel.app` alias가 붙어 있다. 무료 review deploy이며 검색 공개가 아니다. 이전 `v0.2.29`도 같은 경로로 production deploy 됐었다.
...
- 이번 검증 결과: `v0.2.30`은 targeted TDD guard에서 RED/GREEN을 확인했고, local `lint`, `typecheck`, `audit:site`, `build`, `test:site`, `screenshot:site`, `screenshot:admin`, `qa:site`, `npm audit`, `git diff --check`를 통과했다. Source-only audit은 `1590 checks, 1 skipped`, `qa:site` rendered audit은 `1650 checks, 0 skipped`, Playwright는 `25 passed`다. Live `https://kcgold.co.kr`에서도 `check:release-state`, `check:external -- --strict-domain`, `SITE_AUDIT_URL=https://kcgold.co.kr npm run audit:site` (`1650 checks, 0 skipped`), `SITE_AUDIT_URL=https://kcgold.co.kr npm run test:site` (`25 passed`)를 통과했다. 이전 `v0.2.29`은 local `lint`, `typecheck`, `audit:site`, `build`, `test:site`, `npm audit`, `screenshot:site`, `screenshot:admin`, `qa:site`를 통과했다. 회사 Vercel/Supabase 이전은 별도 blocked/shared task로 남긴다.
