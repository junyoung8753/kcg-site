...
- 이번 `v0.2.16`은 회사 계정 기준으로 기존 Vercel/Supabase 프로젝트 이전 가능 범위를 끝까지 확인하고, 공식 문서·회사 CLI·live QA 증거로 남은 일을 분류한 문서/운영 상태 반영 작업이다. 고객 화면, 검색 노출/noindex, 결제/장바구니, 실시간 거래, secret/env, Supabase schema, 실제 제품 데이터는 바꾸지 않는다.
- 되돌릴 때는 "v0.2.16 전으로 되돌려줘" 또는 "v0.2.15 회사 Vercel/Supabase CLI 전환 문서만 되돌리는 계획을 먼저 보여줘"라고 말하면 된다.
...
- Latest change: `Company transfer feasibility and live launch QA`
- Local check URL: `http://127.0.0.1:3300`
- Reflection status: `v0.2.16` is a docs/control-plane pass for company-transfer feasibility and final pre-search live QA. Vercel CLI uses company account `kcgoldx-7259`, team `KCG` (`kcgoldx`) exists, and both company Vercel scopes still have no projects. Supabase CLI sees company-side organizations and target org `Korea Center Gold Exchange` (`raqltqjuqcrusylilnqs`) exists, but the company Supabase project list is empty. The existing live Vercel project `kcg-confirm-preview` and production Supabase project `ehmsqlfxxydnebzjfarr` are not transferred yet, so company CLI cannot deploy/inspect the live Vercel project or link the production Supabase project. Official docs confirm this needs source owner rights and, for Vercel, possibly a target payment method before transfer. Do not restore previous personal CLI sessions for routine KCG operations; transfer or authorize company access first. It preserves `v0.2.12` public customer-flow UI, robots/noindex blocking, Vercel env secrets, Supabase schema, and no checkout/payment/live-trading boundaries. Confirm current state with `git status --short --branch`, `npm run release:trace`, and required validation commands before handoff.
- 실제 사이트 화면이 바뀌는 것: 없음.
- 실제 사이트 화면은 아직 안 바뀌고, 문서/기준만 바뀐 것: `COMPANY_ACCOUNT_MIGRATION_RUNBOOK.md`, `KCG_ACCOUNT_OWNERSHIP_CHECKLIST.md`, handoff/status/task trace, no-secret user-action queue, company-only CLI boundary, official transfer requirements, live external QA evidence.
- 배포된 것: `v0.2.14` minimal company account onboarding mode remains the latest live docs/control-plane deploy. `v0.2.16` is not production deployed because company Vercel CLI has no access to the existing live project until transfer/authorization. Customer-facing UI remains the `v0.2.12` public flow because `v0.2.16` is documentation/control-plane only.
- 아직 배포 안 된 것: `v0.2.16` 문서/기준의 live 반영, 검색 노출/noindex 해제, 실제 상품 운영자료 확정, 유료 서버/API 결제, 기존 Vercel project transfer, 기존 Supabase project transfer, final admin secret rotation.
- 고객에게 보여줘도 되는 것: noindex-protected live `kcgold.co.kr` review site. Search exposure remains blocked.
- 아직 내부 기준/계획일 뿐인 것: real product/store/staff photography replacement, final product prices/공임/판매정책, final admin secret rotation, search launch approval, actual Vercel/Supabase/GitHub/Cafe24 ownership transfer completion, company card entry only when paid server/API billing is needed, optional Workspace/custom-domain mail purchase.
- Latest QA: `v0.2.16` pre-search live external QA passed on 2026-05-06 KST. Passed live: `SITE_AUDIT_URL=https://kcgold.co.kr npm run audit:site` (`1265 checks, 0 skipped`), `SITE_AUDIT_URL=https://kcgold.co.kr npm run test:site` (`21 passed`), `npm run check:external -- --strict-domain`, live `/api/health` (`mode=supabase`, `deployment=production`, `indexing=disabled`, `adminAuth=env-password`), and robots/sitemap checks. Company ownership checks passed: `npx vercel whoami` reports `kcgoldx-7259`, `npx vercel teams ls` shows `KCG`, `npx vercel project ls --scope kcgoldx` and `--scope kcgoldx-7259s-projects` show no projects, `npx supabase orgs list` shows `kcgoldx@gmail.com's Org` and `Korea Center Gold Exchange`, and `npx supabase projects list` shows no company projects. Expected blocked checks: `npx vercel inspect https://kcgold.co.kr/ --scope kcgoldx`, `npx vercel project inspect kcg-confirm-preview --scope kcgoldx`, and `npx supabase link --project-ref ehmsqlfxxydnebzjfarr` fail because the live projects have not been transferred. Public legal source check found approved legal values and no old public phone `02-747-1802`. Last completed public UI QA remains `v0.2.12`, including product-card image variety. Because `v0.2.16` does not change public/admin UI source, screenshot QA is not required unless later source changes are added.
- Change ledger: `docs/setup/CHANGELOG.md`
- Broader rollback bookmark: `backup/pre-v0.2.4-operations-product-audit`
- Rollback phrase: `v0.2.16 전으로 되돌려줘`
