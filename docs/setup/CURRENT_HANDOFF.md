...
- 이번 `v0.2.19`는 기존 Vercel project transfer를 실제 source-owner 세션에서 시도하고, 기존 Vercel 계정 `junyoung8753-2361`을 회사 Vercel team `KCG`의 `MEMBER`로 추가했지만, 무료 team에서 추가 Owner 승격/transfer eligibility가 막힌 상태를 기록한 작업이다. 검색 노출/noindex, 결제/카드 입력, 결제/장바구니, 실시간 거래, SMS 발송, Kakao credential, OpenAI key/env 값, Supabase schema, 실제 제품 데이터, 실제 KRX API 호출은 바꾸지 않는다.
- 되돌릴 때는 "v0.2.19 Vercel 이전 기록만 되돌려줘" 또는 "v0.2.18 상담 도우미 전으로 되돌려줘"라고 말하면 된다.
...
- Latest change: `Vercel transfer owner-member blocker record`
- Local check URL: `http://127.0.0.1:3300`
- Reflection status: `v0.2.19` records the Vercel transfer attempt after `v0.2.18` inquiry-assistant work. `v0.2.18` adds a no-storage public `거래 상담 도우미` on top of `v0.2.17` KRX approval-first guardrails, but it is still not live-deployed. Vercel CLI still uses company account `kcgoldx-7259`; source-owner browser access to `kcg-confirm-preview` was verified, and `junyoung8753-2361` was added to company team `KCG` as `MEMBER`. The transfer modal still returned `No results` for `KCG`/`kcgoldx`, and promoting `junyoung8753-2361` to a second `OWNER` is blocked by Vercel Pro/payment requirements on the free team. The existing live Vercel project and production Supabase project are still not transferred, so company CLI cannot deploy/inspect live project or link production Supabase yet. Do not restore previous personal CLI sessions for routine KCG operations; transfer, add a Vercel-approved payment/eligibility path, or authorize company access first.
...
- 배포된 것: `v0.2.14` minimal company account onboarding mode remains the latest live docs/control-plane deploy. `v0.2.18` and `v0.2.19` are not production deployed because company Vercel CLI has no access to the existing live project until transfer/authorization.
- 아직 배포 안 된 것: `v0.2.18` 상담 도우미, `v0.2.19` transfer blocker docs, inquiry health fields, KRX no-scraping guard docs, KRX approval, KRX provider parser/API call, KRX API key/env entry, 검색 노출/noindex 해제, 실제 상품 운영자료 확정, 유료 서버/API 결제, 기존 Vercel project transfer, 기존 Supabase project transfer, final admin secret rotation.
...
- Latest QA: `v0.2.19` is a docs/operations trace update after a Vercel transfer attempt. Vercel source-owner browser access opened `kcg-confirm-preview` settings and transfer modal; `npx vercel teams invite junyoung8753@gmail.com --scope kcgoldx` added `junyoung8753-2361` to `KCG`; `npx vercel teams members --scope kcgoldx` shows `junyoung8753-2361 MEMBER` and `kcgoldx-7259 OWNER`. Expected blocked: free-team Owner promotion failed with Vercel's Pro requirement, transfer target search returned `No results` for `KCG`/`kcgoldx`, and temporary source CLI login kept resolving to company account before being logged out. `v0.2.18` verification previously passed after code changes: `npm run qa:site` completed rendered audit with `1381 checks, 0 skipped`, browser tests `22 passed`, and npm audit `0 vulnerabilities`; `npm run check:external -- --strict-domain` confirmed live health remains `indexing=disabled`, robots blocked, sitemap empty, and both KCG domains on Vercel. Current live test mismatch remains expected until deployment: live Playwright passes 21 tests and fails only the `v0.2.18` inquiry-assistant button test because `v0.2.18` is not deployed.
- Change ledger: `docs/setup/CHANGELOG.md`
- Broader rollback bookmark: `backup/pre-v0.2.4-operations-product-audit`
- Rollback phrase: `v0.2.19 Vercel 이전 기록만 되돌려줘`
