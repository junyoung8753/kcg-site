...
- 배포된 것: `v0.2.29`가 기존 personal Hobby Vercel project `kcg-confirm-preview`로 production deployed 됐다. `v0.2.30`은 final local QA를 통과했고 기존 live review domains로 배포하는 단계다. Use `npx vercel inspect https://kcgold.co.kr/` for the current deployment id. 이 deploy는 noindex-protected review publication이며 search launch가 아니다.
