...
- 이번 `v0.2.18`은 고객이 전화하기 전 기본 질문을 줄이기 위해 저장 없는 `거래 상담 도우미`를 추가하고, KRX 우회 수집·hidden endpoint·TradingView 추출·경쟁사 가격 복사를 계속 막은 작업이다. 검색 노출/noindex, 결제/장바구니, 실시간 거래, SMS 발송, Kakao credential, OpenAI key/env 값, Supabase schema, 실제 제품 데이터, 실제 KRX API 호출은 바꾸지 않는다.
- 되돌릴 때는 "v0.2.18 상담 도우미 전으로 되돌려줘" 또는 "v0.2.17 KRX 승인 우선 guard만 되돌리는 계획을 먼저 보여줘"라고 말하면 된다.
...
- Latest change: `Inquiry assistant and KRX no-scraping boundary`
- Local check URL: `http://127.0.0.1:3300`
- Reflection status: `v0.2.18` adds a no-storage public `거래 상담 도우미` on top of `v0.2.17` KRX approval-first guardrails. The assistant answers only approved FAQ/visit/price/KRX-boundary questions by default rules, can optionally use OpenAI Responses API later with `store: false`, and does not store messages or collect contact details. Phone/email/resident-number inputs, final-price requests, payment/trading, legal/tax, SMS/Kakao automation, and staff-required questions are routed to human consultation. KRX/Koscom remains blocked until approval; KRX hidden endpoints, TradingView extraction, and competitor price copying are not allowed. Vercel CLI still uses company account `kcgoldx-7259`; the existing live Vercel project and production Supabase project are still not transferred, so company CLI cannot deploy/inspect live project or link production Supabase yet. Do not restore previous personal CLI sessions for routine KCG operations; transfer or authorize company access first.
- 실제 사이트 화면이 바뀌는 것: 데스크톱 오른쪽 하단 `상담 도우미` 버튼, 모바일 하단 고정 CTA의 `상담` 버튼, and the dialog they open. `/api/health` also exposes inquiry assistant status fields.
- 실제 사이트 화면은 아직 안 바뀌고, 문서/기준만 바뀐 것: OpenAI API key, SMS staff alert provider, official Kakao channel URL, and KRX/Koscom approval/contract steps are prepared only as later user-only/provider steps.
- 배포된 것: `v0.2.14` minimal company account onboarding mode remains the latest live docs/control-plane deploy. `v0.2.18` is not production deployed because company Vercel CLI has no access to the existing live project until transfer/authorization.
- 아직 배포 안 된 것: `v0.2.18` 상담 도우미, inquiry health fields, KRX no-scraping guard docs, KRX approval, KRX provider parser/API call, KRX API key/env entry, 검색 노출/noindex 해제, 실제 상품 운영자료 확정, 유료 서버/API 결제, 기존 Vercel project transfer, 기존 Supabase project transfer, final admin secret rotation.
- 고객에게 보여줘도 되는 것: noindex-protected live `kcgold.co.kr` review site. Search exposure remains blocked.
- 아직 내부 기준/계획일 뿐인 것: OpenAI API key/billing, Kakao official channel URL, SMS provider contract/key, KRX/Koscom approval result, final KRX attribution wording, KRX public/commercial display scope, real product/store/staff photography replacement, final product prices/공임/판매정책, final admin secret rotation, search launch approval, actual Vercel/Supabase/GitHub/Cafe24 ownership transfer completion, company card entry only when paid server/API billing is needed, optional Workspace/custom-domain mail purchase.
- Latest QA: `v0.2.18` verification passed in the current task after code changes: `npm run qa:site` completed rendered audit with `1381 checks, 0 skipped`, browser tests `22 passed`, and npm audit `0 vulnerabilities`; `git diff --check` passed with line-ending warnings only; `npm run check:external -- --strict-domain` confirmed live health remains `indexing=disabled`, robots blocked, sitemap empty, and both KCG domains on Vercel. Manual screenshot inspection covered `home-mobile-viewport.png`, `prices-mobile-viewport.png`, `home-desktop-viewport.png`, and `inquiry-assistant-mobile-open.png`. Last completed public UI QA for product-card image variety remains the `v0.2.12` product/catalog screenshot pass, and last live external rendered audit for the currently deployed site remains `v0.2.16` pre-search live QA on 2026-05-06 KST with `1265 checks, 0 skipped` because company Vercel `kcgoldx` still cannot deploy/inspect the existing live project. Company ownership checks remain blocked as expected: `npx vercel whoami` reports `kcgoldx-7259`, `npx vercel project ls --scope kcgoldx` shows no projects, and `npx vercel inspect https://kcgold.co.kr/ --scope kcgoldx` fails until the existing live project is transferred or company access is granted. Vercel/Supabase transfer still requires source owner rights or equivalent company access from the current source owners.
- Change ledger: `docs/setup/CHANGELOG.md`
- Broader rollback bookmark: `backup/pre-v0.2.4-operations-product-audit`
- Rollback phrase: `v0.2.18 상담 도우미 전으로 되돌려줘`
