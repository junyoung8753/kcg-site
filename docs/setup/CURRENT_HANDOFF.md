...
- 이번 `v0.2.20`은 검색 공개가 차단 플래그 부재만으로 열리지 않도록 `KCG_PUBLIC_SEARCH_APPROVED=1` 명시 승인 gate를 추가하고, live/source 버전 불일치를 분류하는 `npm run check:release-state`를 추가한 작업이다. 검색 노출/noindex release, 결제/카드 입력, 결제/장바구니, 실시간 거래, SMS 발송, Kakao credential, OpenAI key/env 값, Supabase schema, 실제 제품 데이터, 실제 KRX API 호출은 바꾸지 않는다.
- 되돌릴 때는 "v0.2.20 전으로 되돌려줘", "v0.2.19 Vercel 이전 기록만 되돌려줘", 또는 "v0.2.18 상담 도우미 전으로 되돌려줘"라고 말하면 된다.
...
- Latest change: `Search approval guard and release-state QA`
- Local check URL: `http://127.0.0.1:3300`
- Reflection status: `v0.2.20` adds an explicit public-search approval gate after the `v0.2.19` Vercel transfer blocker record and `v0.2.18` inquiry-assistant work. Search exposure now requires `KCG_PUBLIC_SEARCH_APPROVED=1` in addition to production, final domain, no legal blockers, and no forced noindex. `npm run check:release-state` classifies live/source version mismatch when live lacks newer source health fields. The existing live Vercel project and production Supabase project are still not transferred, so company CLI cannot deploy/inspect live project or link production Supabase yet. Do not restore previous personal CLI sessions for routine KCG operations; transfer, add a Vercel-approved payment/eligibility path, or authorize company access first.
- 실제 사이트 화면이 바뀌는 것: `/admin/launch` 공개 승인 checklist에 `KCG_PUBLIC_SEARCH_APPROVED=1` 단계가 추가되고, admin product form label `임시 공임`이 `상담 기준 공임`으로 바뀐다. `v0.2.18` source에는 데스크톱 오른쪽 하단 `상담 도우미` 버튼, 모바일 하단 고정 CTA의 `상담` 버튼, and the dialog they open도 포함되어 있다.
...
- 배포된 것: `v0.2.14` minimal company account onboarding mode remains the latest live docs/control-plane deploy. `v0.2.18`, `v0.2.19`, and `v0.2.20` are not production deployed while company Vercel CLI has no access to the existing live project until transfer/authorization.
- 아직 배포 안 된 것: `v0.2.18` 상담 도우미, `v0.2.19` transfer blocker docs, `v0.2.20` search-approval health/admin guard and release-state checker, inquiry health fields, KRX no-scraping guard docs, KRX approval, KRX provider parser/API call, KRX API key/env entry, 검색 노출/noindex 해제, 실제 상품 운영자료 확정, 유료 서버/API 결제, 기존 Vercel project transfer, 기존 Supabase project transfer, final admin secret rotation.
...
- Latest QA: `v0.2.20` passed `npm run lint`, `npm run typecheck`, `npm run audit:site` (`1347 checks, 1 skipped`), `npm run build`, `npm run test:site` (`22 passed`), `npm run screenshot:site`, `npm run screenshot:admin`, `npm run qa:site` (`1407 checks, 0 skipped`; Playwright `22 passed`; npm audit `0 vulnerabilities`), standalone `npm audit --audit-level=moderate`, and `git diff --check` with line-ending warnings only. `npm run check:release-state` confirmed live is reachable and still not search-indexable while classifying missing `v0.2.18+`/`v0.2.20+` health fields as live-behind-source. Live external QA passed with `SITE_AUDIT_URL=https://kcgold.co.kr npm run audit:site` (`1407 checks, 0 skipped`) and `npm run check:external -- --strict-domain`. Company Vercel is still blocked: `npx vercel whoami` reports `kcgoldx-7259`, team `KCG` exists, but `npx vercel project ls --scope kcgoldx` returns no projects and `npx vercel inspect https://kcgold.co.kr/ --scope kcgoldx` cannot find the deployment. Company Supabase org `Korea Center Gold Exchange` exists but `npx supabase projects list` shows no projects.
- Change ledger: `docs/setup/CHANGELOG.md`
- Broader rollback bookmark: `backup/pre-v0.2.4-operations-product-audit`
- Rollback phrase: `v0.2.20 전으로 되돌려줘`
...
- Search exposure must use `canExposeToSearch()` rather than raw production/noindex state. As of `v0.2.20`, it requires explicit `KCG_PUBLIC_SEARCH_APPROVED=1` in addition to final domain, confirmed legal information, production deployment, and no forced noindex, so missing approval keeps robots/sitemap blocked.
