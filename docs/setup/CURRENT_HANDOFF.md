...
- 이번 `v0.2.29`는 Junyoung이 준 KCG 골드바 참고 폴더를 원본 그대로 쓰지 않고, 골드바 문양·크기·라인업 느낌을 바탕으로 새 생성 WebP 3종을 만들어 홈 배너, 상품 hero/promo, 골드바 상품 대표 이미지에 연결한 작업이다. 이미지는 repo/public에 생성 대표 자산으로 들어갔지만 raw KakaoTalk 원본은 복사하지 않았고, 실제 상품사진/포장/재고/최종 제품 증거로 확정하지 않았다. 검색 노출/noindex release, 결제/카드 입력, 결제/장바구니, 실시간 거래, SMS 발송, Kakao credential, OpenAI key/env 값, Supabase schema, 실제 제품 가격/데이터, 실제 KRX API 호출은 바꾸지 않는다.
- 되돌릴 때는 "v0.2.29 전으로 되돌려줘", "v0.2.28 대표 이미지 안내 전으로 되돌려줘", "v0.2.27 이미지 replacement map 전으로 되돌려줘", "v0.2.26 이미지 intake guardrail 전으로 되돌려줘", "v0.2.25 모바일 이미지 근거 전으로 되돌려줘", "v0.2.24 이미지 확인 필터 전으로 되돌려줘", "v0.2.23 운영 상태 구분 전으로 되돌려줘", "v0.2.22 이미지 성격 전으로 되돌려줘", 또는 "v0.2.18 상담 도우미 전으로 되돌려줘"라고 말하면 된다.
...
- Latest change: `Generated KCG gold-bar representative assets`
...
- 배포된 것: `v0.2.29`가 기존 personal Hobby Vercel project `kcg-confirm-preview`로 production deployed 됐다. Use `npx vercel inspect https://kcgold.co.kr/` for the current deployment id. 이 deploy는 noindex-protected review publication이며 search launch가 아니다.
...
- Latest QA: `v0.2.29` local QA passed (`lint`, `typecheck`, `audit:site`, `build`, `test:site`, `npm audit`, `screenshot:site`, `screenshot:admin`, and `qa:site` with rendered audit `1603 checks, 0 skipped`). Production deploy verification passed on the restored personal Vercel free path: `npx vercel whoami` reports `junyoung8753-2361`, `npx vercel project inspect kcg-confirm-preview` finds project `prj_lqsFijkBcN7BAadaGygNlWy416W0`, `npx vercel inspect https://kcgold.co.kr/` finds the production deployment ready, `npm run check:release-state` passes with search exposure guarded, `npm run check:external -- --strict-domain` passes, and live `SITE_AUDIT_URL=https://kcgold.co.kr npm run audit:site` plus live `SITE_AUDIT_URL=https://kcgold.co.kr npm run test:site` pass. Live Playwright initially exposed production Supabase legacy-image repetition, so generated representative placeholder URLs are now also slug-remappable before the final live test rerun.
- Change ledger: `docs/setup/CHANGELOG.md`
- Broader rollback bookmark: `backup/pre-v0.2.4-operations-product-audit`
- Rollback phrase: `v0.2.29 전으로 되돌려줘`
