...
- 이번 `v0.2.15`는 기존 개인 Vercel/Supabase CLI 세션을 운영 경로에서 제외하고, `kcgoldx@gmail.com` 회사 계정 기준으로 Vercel 팀 `KCG`와 Supabase 조직 `Korea Center Gold Exchange`를 실제 생성한 문서/운영 상태 반영 작업이다. 고객 화면, 검색 노출/noindex, 결제/장바구니, 실시간 거래, secret/env는 바꾸지 않는다.
- 되돌릴 때는 "v0.2.15 전으로 되돌려줘" 또는 "v0.2.14 최소 회사 계정 온보딩 모드 문서만 되돌리는 계획을 먼저 보여줘"라고 말하면 된다.
...
- Latest change: `Company Vercel and Supabase ownership start`
- Local check URL: `http://127.0.0.1:3300`
- Reflection status: `v0.2.15` is a docs/control-plane pass for the company-account transition. Vercel CLI now uses company account `kcgoldx-7259` and team `KCG` (`kcgoldx`) exists. Supabase CLI now sees company-side organizations and target org `Korea Center Gold Exchange` (`raqltqjuqcrusylilnqs`) exists. The existing live Vercel project `kcg-confirm-preview` and production Supabase project `ehmsqlfxxydnebzjfarr` are not transferred yet, so company CLI cannot deploy/inspect the live Vercel project or list the production Supabase project. Do not restore previous personal CLI sessions for routine KCG operations; transfer or authorize company access first. It preserves `v0.2.12` public customer-flow UI, robots/noindex blocking, Vercel env secrets, and no checkout/payment/live-trading boundaries. Confirm current state with `git status --short --branch`, `npm run release:trace`, and required validation commands before handoff.
- 실제 사이트 화면이 바뀌는 것: 없음.
- 실제 사이트 화면은 아직 안 바뀌고, 문서/기준만 바뀐 것: `COMPANY_ACCOUNT_MIGRATION_RUNBOOK.md`, `KCG_ACCOUNT_OWNERSHIP_CHECKLIST.md`, handoff/status/task trace, no-secret user-action queue, company-only CLI boundary.
- 배포된 것: `v0.2.14` minimal company account onboarding mode remains the latest live docs/control-plane deploy. `v0.2.15` is not production deployed because company Vercel CLI has no access to the existing live project until transfer/authorization. Customer-facing UI remains the `v0.2.12` public flow because `v0.2.15` is documentation/control-plane only.
- 아직 배포 안 된 것: `v0.2.15` 문서/기준의 live 반영, 검색 노출/noindex 해제, 실제 상품 운영자료 확정, 유료 서버/API 결제, 기존 Vercel project transfer, 기존 Supabase project transfer.
...
- Latest QA: local `v0.2.15` docs/control-plane validation passed on 2026-05-06 KST. Passed: `npm run audit:site` (`1205 checks, 1 skipped` because rendered URL checks were not requested), `npm run release:trace`, `npm run tasks:dashboard` (`67 tasks`), `npm run lint`, `npm run typecheck`, `npm run build`, `npm run test:site` (`21 passed`), `npm audit --audit-level=moderate` (`0 vulnerabilities`), `git diff --check` with line-ending warnings only, and `npm run check:external -- --strict-domain`. Company ownership checks passed: `npx vercel whoami` reports `kcgoldx-7259`, `npx vercel teams ls` shows `KCG`, `npx vercel project ls --scope kcgoldx` and `--scope kcgoldx-7259s-projects` show no projects, `npx supabase orgs list` shows `kcgoldx@gmail.com's Org` and `Korea Center Gold Exchange`, and `npx supabase projects list` shows no company projects. Expected blocked check: `npx vercel inspect https://kcgold.co.kr/ --scope kcgoldx` fails because the live project has not been transferred. Last completed public UI QA remains `v0.2.12`, including product-card image variety. Because `v0.2.15` does not change public/admin UI source, screenshot QA is not required unless later source changes are added.
- Change ledger: `docs/setup/CHANGELOG.md`
- Broader rollback bookmark: `backup/pre-v0.2.4-operations-product-audit`
- Rollback phrase: `v0.2.15 전으로 되돌려줘`
