...
- 배포된 것: final QA 후 `v0.2.29`를 기존 personal Hobby Vercel project `kcg-confirm-preview`로 production deploy한다. 이 deploy는 review publication이며 search launch가 아니다.
...
- Latest QA: pending final `v0.2.29` full QA and production deploy verification in the current pass. Personal Vercel free deploy access was restored: `npx vercel whoami` reports `junyoung8753-2361`, `npx vercel project inspect kcg-confirm-preview` finds project `prj_lqsFijkBcN7BAadaGygNlWy416W0`, and `npx vercel inspect https://kcgold.co.kr/` finds production deployment `dpl_3QU9KSucBFZbtv2EAv3PkLvPraC2` before the new deploy.
