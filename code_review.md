...
- Main price disclosure: the homepage first viewport must show the KCG company price table or a clear price-check path before campaign images, charts, or market-reference widgets take priority.
...
- Existing API integration: already-connected APIs are current-site QA scope. Review source hierarchy, update time, stale/failure fallback, rate-limit/timeout/empty response handling, and whether API values can be mistaken for final KCG posted prices.
- Image / visual asset strategy: distinguish real KCG product photos from generated/placeholder assets, track real-photo replacement tasks, and verify images support trust/conversion without pushing the price table down.
...
- If the diff touches API data, are existing API surfaces audited as P0/P1 rather than deferred as a vague future task?
- If the diff touches images, is every customer-decision image classified as real photo, generated/placeholder, stock, logo, or internal-only material?
- If the diff touches price surfaces, is company posted price still the source of truth and visually ahead of API/international reference data?
